...
# Loading data ------------------------------------------------------------
encuesta_mrp <- readRDS("MrPDepartamental/DOM/2020/Data/encuesta_mrp.rds") %>% 
  filter(edad != "1")
censo_mrp <- readRDS("MrPDepartamental/DOM/2020/Data/censo_mrp.rds") %>% 
  filter(edad != "1")
tasa_desocupados <- readRDS("MrPDepartamental/DOM/2020/Data/tasa_desocupacion.rds")

fit <- readRDS(file = "MrPDepartamental/DOM/2020/Data/fit_multinomial.rds")
...
byAgrega <-
  grep(
    pattern =  "^(n|pobreza|ingreso|lp|li|fep|trimestre)",
...
if(any(names(Total) != names(Xdommy))){
  ...
  data.frame(Poblacion = colSums(Xdommy * poststrat_df$n),
             Muestra = Total) %>% 
    mutate(diff = abs(Poblacion - Muestra)) %>%
    arrange(diff)
}
...
data.frame(Poblacion = colSums(Xdommy*poststrat_df$n*gk),
           Muestra = Total) %>% mutate(diff = abs(Poblacion- Muestra)) %>% 
  arrange(diff)


jpeg(file = "MrPDepartamental/DOM/2020/Output/Plot_Bench_gk.jpeg", width = 1200,
     height = 1200)
hist(gk)
dev.off()
poststrat_df$gk <- gk
...
saveRDS(poststrat_df %>% select(!matches("_\\d{,2}$"), matches("theta_")), 
        "MrPDepartamental/DOM/2020/Data/poststrat_df.RDS")
