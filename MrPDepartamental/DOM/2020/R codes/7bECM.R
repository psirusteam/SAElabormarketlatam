...
poststrat_df <- readRDS("DOM/2020/1.Ingreso/Data/poststrat_df.RDS") %>% 
  filter(anoest != "99")
tasa_desocupados <- readRDS("DOM/2020/1.Ingreso/Data/tasa_desocupacion.rds")
fit <- readRDS("DOM/2020/1.Ingreso/Data/fit_bayes.rds")
...
(lp <- encuesta_mrp %>% select(area, lp) %>% unique() %>% 
   group_by(area) %>% summarise(lp = mean(lp)))
...
openxlsx::write.xlsx(tablas, file = "DOM/2020/1.Ingreso/Output/tablas_ECM.xlsx", 
                     overwrite = TRUE)
