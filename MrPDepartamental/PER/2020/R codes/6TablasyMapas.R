...
ShapeSAE <- read_sf("MrPDepartamental/PER/2020/ShapeDeptoPER/DEPARTAMENTOS.shp")

ShapeSAE %<>% mutate(depto = str_pad(IDDPTO, pad = "0", width = 2))
...
cod_shape <- ShapeSAE %>% as.data.frame() %>% select(depto, DEPARTAMEN) %>%
  arrange(depto)
...
cod_censo <- c(
  ...
  mutate(nombre = toupper(nombre),
         nombre = iconv(nombre, "UTF-8", "latin1") )
...
openxlsx::write.xlsx(dat_df, 
                     file = "MrPDepartamental/PER/2020/Output/tablas.xlsx", 
                     overwrite = TRUE)
openxlsx::openXL("MrPDepartamental/PER/2020/Output/tablas.xlsx")
