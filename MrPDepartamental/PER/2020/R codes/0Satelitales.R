...
####################################################
tasa_desocupacion <-
  readRDS("PER/2020/1.Ingreso/Data/tasa_desocupacion.rds")
...
## revisando PERentina
PER <- read_sf("PER/ShapeDeptoPER/DEPARTAMENTOS.shp")
PER %<>% mutate(depto = str_pad(IDDPTO, pad = "0", width = 2))
...
PER_luces <- map(unique(PER$depto),
                 ~tryCatch(ee_extract(
                   x = luces,
                   y = PER["depto"] %>% filter(depto == .x),
                   ee$Reducer$mean(),
                   sf = FALSE
...
PER_urbano_cultivo <- map(unique(PER$depto),
                 ~tryCatch(ee_extract(
                   x = tiposuelo,
                   y = PER["depto"] %>% filter(depto == .x),
                   ee$Reducer$mean(),
...
saveRDS(tasa_desocupacion, "PER/2020/1.Ingreso/Data/tasa_desocupacion.rds")
