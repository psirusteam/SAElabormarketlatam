...
censo_mrp <- readRDS("MrPDepartamental/HND/2019/Data/censo_mrp.rds") %>% 
  filter(edad != "1")
tasa_desocupados <- readRDS("MrPDepartamental/HND/2019/Data/tasa_desocupacion.rds")

fit <- readRDS(file = "MrPDepartamental/HND/2019/Data/fit_multinomial.rds")
...
jpeg(file = "MrPDepartamental/HND/2019/Output/Plot_Bench_gk.jpeg", width = 1200)
hist(gk)
dev.off()
poststrat_df$gk <- gk
saveRDS(poststrat_df %>% select(!matches("_\\d{,2}$"), matches("theta_")), 
        "MrPDepartamental/HND/2019/Data/poststrat_df.RDS")
