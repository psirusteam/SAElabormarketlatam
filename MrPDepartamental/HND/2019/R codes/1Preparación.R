...
####################################################
encuesta <- readRDS("MrPDepartamental/HND/2019/Data/encuestaHND19n.rds")
censo_mrp <- readRDS( "MrPDepartamental/HND/2019/Data/censo_mrp.rds")
...
encuesta <-
  unique(encuesta %>% select(id_hogar, depto)) %>% na.omit() %>%
  inner_join(encuesta %>% select(-depto), by = "id_hogar")
...
encuesta_mrp <- encuesta %>% 
  ...
  anoest = case_when(
    edad < 6 | anoest == -1 | is.na(anoest)  ~ "98"  , #No aplica
    anoest == 99 ~ "99", #NS/NR
...
saveRDS(encuesta_mrp, file = "MrPDepartamental/HND/2019/Data/encuesta_mrp.rds")
...
jpeg(filename = "MrPDepartamental/HND/2019/Output/plot_actualizacion_censo1.jpeg",
...
jpeg(filename = "MrPDepartamental/HND/2019/Output/plot_actualizacion_censo2.jpeg",
     width = 2000, height = 2000)
par(mfrow = c(2,2))
hist(censo_mrp$n)
boxplot(censo_mrp$n)
...
saveRDS(censo_mrp, "MrPDepartamental/HND/2019/Data/censo_mrp.rds")
