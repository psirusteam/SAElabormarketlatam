...
####################################################
encuesta <- readRDS("MrPDepartamental/GTM/2014/1.Ingreso/Data/encuestaGTM14n.rds")
censo_mrp <- readRDS( "MrPDepartamental/GTM/2014/1.Ingreso/Data/censo_mrp.rds")
...
encuesta_mrp <- encuesta %>%
  filter(edad > 6, condact3 %in% 1:3)  %>% 
  transmute(
  depto = str_pad(string = depto,width = 2, pad = "0"),
  ingreso = ingcorte, 
  empleo = condact3,
...
saveRDS(encuesta_mrp, file = "MrPDepartamental/GTM/2014/1.Ingreso/Data/encuesta_mrp.rds")
...
## la encuesta estima más personas que el censo. 
n1 <- ceiling(censo_mrp$n*gk)
summary(n1)
summary(censo_mrp$n)

jpeg(filename = "MrPDepartamental/GTM/2014/1.Ingreso/Output/plot_actualizacion_censo1.jpeg",
...
jpeg(filename = "MrPDepartamental/GTM/2014/1.Ingreso/Output/plot_actualizacion_censo2.jpeg",
     width = 2000, height = 2000)
par(mfrow = c(2,2))
hist(censo_mrp$n)
boxplot(censo_mrp$n)
