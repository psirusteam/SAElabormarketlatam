...
encuesta_mrp <- readRDS("MrPDepartamental/GTM/2014/Data/encuesta_mrp.rds") %>% 
  filter(edad != "1")
censo_mrp <- readRDS("MrPDepartamental/GTM/2014/Data/censo_mrp.rds") %>% 
  filter(edad != "1")
tasa_desocupados <- readRDS("MrPDepartamental/GTM/2014/Data/tasa_desocupacion.rds")

fit <- readRDS(file = "MrPDepartamental/GTM/2014/Data/fit_multinomial.rds")
...
jpeg(file = "MrPDepartamental/GTM/2014/Output/Plot_Bench_gk.jpeg", width = 1200)
hist(gk)
dev.off()
poststrat_df$gk <- gk

saveRDS(poststrat_df %>% select(!matches("_\\d{,2}$"), matches("theta_")), 
        "MrPDepartamental/GTM/2014/Data/poststrat_df.RDS")
