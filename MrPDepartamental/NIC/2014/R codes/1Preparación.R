...
####################################################
encuesta <- readRDS("MrPDepartamental/NIC/2014/1.Ingreso/Data/encuestaNIC14N.rds")
censo_mrp <- readRDS( "MrPDepartamental/NIC/2014/1.Ingreso/Data/censo_mrp.rds")

##############################
### Exploratory statistics ###
##############################
  encuesta %>% group_by(dominio4, condact3) %>%
...
table(encuesta$anoest, useNA = "always") %>% data.frame() %>%
  mutate(Prop = round(Freq/ sum(Freq),2), Acum = cumsum(Prop))
...
saveRDS(encuesta_mrp, file = "MrPDepartamental/NIC/2014/1.Ingreso/Data/encuesta_mrp.rds")
...
jpeg(filename = "MrPDepartamental/NIC/2014/1.Ingreso/Output/plot_actualizacion_censo1.jpeg",
...
jpeg(filename = "MrPDepartamental/NIC/2014/1.Ingreso/Output/plot_actualizacion_censo2.jpeg",
     width = 2000, height = 2000)
par(mfrow = c(2,2))
hist(censo_mrp$n)
boxplot(censo_mrp$n)
