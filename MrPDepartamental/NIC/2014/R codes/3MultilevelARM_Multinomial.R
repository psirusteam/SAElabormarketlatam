...
##################################################
X <- encuesta_df_agg %>% select(-matches("anoest|edad|etnia|depto|empleo|tasa|^F|^X")) %>%
  model.matrix( ~ -1+ ., data = .)  %>%
...
Z <- encuesta_df_agg %>% select(matches("anoest|edad|etnia")) %>%
  model.matrix( ~ -1+ ., data = .)%>%
  as.matrix(.)


Xp <- censo_df %>% select(-matches("anoest|edad|etnia|^n|depto|empleo|tasa|^F|^X")) %>%
  model.matrix( ~-1+ ., data = .)  %>%
  bind_cols(censo_df %>% select(matches("tasa|F|^X"))) 

Zp <- censo_df %>% select(matches("anoest|edad|etnia")) %>%
  model.matrix( ~ -1+ ., data = .)%>%
  as.matrix(.)
...
fit_mcmc$save_object( 
        file = "MrPDepartamental/NIC/2014/Data/fit_multinomial.rds")
