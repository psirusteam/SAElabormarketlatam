...
encuesta_mrp <- readRDS("MrPDepartamental/NIC/2014/Data/encuesta_mrp.rds") %>% 
  filter(edad != "1")
censo_mrp <- readRDS("MrPDepartamental/NIC/2014/Data/censo_mrp.rds") %>% 
  filter(edad != "1")
tasa_desocupados <- readRDS("MrPDepartamental/NIC/2014/Data/tasa_desocupacion.rds")

fit <- readRDS(file = "MrPDepartamental/NIC/2014/Data/fit_multinomial.rds")
...
# draws_temp <- fit_temp$generate_quantities(fit,sample_data, parallel_chains = 4)
# draws_temp <-  as_draws_df(draws_temp$draws())
theta_censo <- draws_temp %>% select(matches("theta_p\\[")) %>% 
  colMeans() %>% 
...
jpeg(file = "MrPDepartamental/NIC/2014/Output/Plot_Bench_gk.jpeg", 
...
saveRDS(poststrat_df %>% select(!matches("_\\d{,2}$"), matches("theta_")), 
        "MrPDepartamental/NIC/2014/Data/poststrat_df.RDS")
