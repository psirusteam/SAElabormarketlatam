...
encuesta_mrp <- readRDS("NIC/2014/1.Ingreso/Data/encuesta_mrp.rds")
poststrat_df <- readRDS("NIC/2014/1.Ingreso/Data/poststrat_df.RDS") %>% 
  filter(anoest != "99")
tasa_desocupados <- readRDS("NIC/2014/1.Ingreso/Data/tasa_desocupacion.rds")
fit <- readRDS("NIC/2014/1.Ingreso/Data/fit_bayes.rds")
...
byAgrega <-
  grep(
    pattern =  "^(n|pobreza|ingreso|tasa_desocupacion|epred_mat|gk|depto|lp)",
...
openxlsx::write.xlsx(tablas, file = "NIC/2014/1.Ingreso/Output/tablas_ECM.xlsx", 
                     overwrite = TRUE)
