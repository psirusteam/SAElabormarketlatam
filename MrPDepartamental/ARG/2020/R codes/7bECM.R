...
encuesta_mrp <- readRDS("ARG/2020/1.Ingreso/Data/encuesta_mrp.rds")
poststrat_df <- readRDS("ARG/2020/1.Ingreso/Data/poststrat_df.RDS") %>% 
  filter(anoest != "99")
tasa_desocupados <- readRDS("ARG/2020/1.Ingreso/Data/tasa_desocupacion.rds")
fit <- readRDS("ARG/2020/1.Ingreso/Data/fit_bayes.rds")
...
byAgrega <-
  grep(
    pattern =  "^(n|pobreza|ingreso|tasa_desocupacion|epred_mat|gk|depto|lp|X|F)",
...
(lp <- encuesta_mrp %>% select( lp) %>% unique())
...
openxlsx::write.xlsx(tablas, file = "ARG/2020/1.Ingreso/Output/tablas_ECM.xlsx", 
                     overwrite = TRUE)
