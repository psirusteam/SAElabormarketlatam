...
fit <- readRDS(file = "MrPDepartamental/ARG/2020/Data/fit_multinomial.rds")
...
jpeg(file = "MrPDepartamental/ARG/2020/Output/Plot_Bench_gk.jpeg")
hist(gk)
dev.off()
poststrat_df$gk <- gk

saveRDS(poststrat_df %>% select(!matches("_\\d{,2}$"), matches("theta_")), 
        "MrPDepartamental/ARG/2020/Data/poststrat_df.RDS")
