...
Paraguay <- redatam.open("MrPDepartamental/PRY2/2020/Data/cpv2002pry-cde.dicx")
...
saveRDS(CONTEOS, file = "MrPDepartamental/PRY2/2020/Data/CONTEOS.RDS")
rm("$table1")
# CONTEOS <- readRDS(file = "MrPDepartamental/PRY2/2020/Data/CONTEOS.RDS")
...
censo_mrp <- CONTEOS2 %>% 
  ...
  transmute(depto = str_pad(string = IDPTO1_value,
            ...
            value) %>% 
  mutate(depto2 = case_when(
    depto %in% c("00", "02", "05", "06", "07", "10", "11") ~ depto,
    TRUE ~ "20"
  )) %>% 
  group_by(depto,depto2, area, sexo, edad, anoest) %>%
...
map(c("depto", "depto2", "area",  "sexo", "edad", "anoest"),
    function(x){
...
OCUPACION2_2 <- OCUPACION2 %>%  
  group_by(depto, depto2, ocupados, desocupados) %>%
...
tasa_desocupacion = tabla %>% data.frame() %>% 
  transmute(depto, depto2,
            tasa_desocupacion2 = ocupados0_1 / sum(ocupados0_1 + ocupados1_0))

## Agregado
OCUPACION2_2 <- OCUPACION2 %>%  
  group_by(depto2, ocupados, desocupados) %>%
  summarise(value = sum(value), .groups = "drop")
...
tasa_desocupacion2 = tabla %>% data.frame() %>% 
  transmute(depto2,
            tasa_desocupacion = ocupados0_1 / sum(ocupados0_1 + ocupados1_0))

tasa_desocupacion <- full_join( tasa_desocupacion2,
                                tasa_desocupacion, by = c("depto2" = "depto2")) 
...
dptorep <- read_dta("V:/DAT/BADEHOG_N/BG/pry20n/pry20n.dta", encoding = "latin1")$dptorep
encuesta <- read_dta("V:/DAT/BADEHOG_N/BC/PRY_2020N.dta", encoding = "latin1") %>% 
  mutate(dptorep = dptorep)

## Guardar encuesta
saveRDS(encuesta, "MrPDepartamental/PRY2/2020/Data/encuestaPRY20n.rds")
