...
statelevel_predictors_df20 <- tasa_desocupados %>% 
  select(!ends_with("2"), -depto, depto2) %>% 
  distinct() %>% filter(depto2 == "20") %>% 
  rename(depto = depto2) %>% 
  rename_at(vars(!matches("depto")), function(x) paste0(x,"2"))

statelevel_predictors_df %<>% bind_rows(statelevel_predictors_df20) 
...
fit_mcmc$save_object( 
        file = "MrPDepartamental/PRY2/2020/Data/fit_multinomial.rds")
