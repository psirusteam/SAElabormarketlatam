...
ShapeSAE <- read_sf("MrPDepartamental/CRI/2020/ShapeDeptoCRI/regiones de mideplan_CR.shp")

ShapeSAE %<>% mutate(depto = str_pad(codigo, pad = "0", width = 2),
                     depto = case_when(depto == "02" ~ "03",
                                       depto == "03" ~ "02",
                                       depto == "04" ~ "06",
                                       depto == "06" ~ "04",
                                       TRUE ~ depto))
...
cod_shape <- ShapeSAE %>% as.data.frame() %>% select(depto, Regiones) %>%
  arrange(depto)
...
openxlsx::write.xlsx(dat_df, 
                     file = "MrPDepartamental/CRI/2020/Output/tablas.xlsx", 
                     overwrite = TRUE)
openxlsx::openXL("MrPDepartamental/CRI/2020/Output/tablas.xlsx")
