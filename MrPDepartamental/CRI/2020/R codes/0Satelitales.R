...
####################################################
tasa_desocupacion <-
  readRDS("CRI/2020/1.Ingreso/Data/tasa_desocupacion.rds")
...
## revisando CRIivia
CRI <- read_sf("CRI/ShapeDeptoCRI/regiones de mideplan_CR.shp")

CRI %<>% mutate(depto = str_pad(codigo, pad = "0", width = 2),
...
tasa_desocupacion %<>% 
  full_join(CRI_luces) %>% 
  full_join(CRI_urbano_cultivo)

cor(tasa_desocupacion[, -1 ])

saveRDS(tasa_desocupacion, "CRI/2020/1.Ingreso/Data/tasa_desocupacion.rds")
