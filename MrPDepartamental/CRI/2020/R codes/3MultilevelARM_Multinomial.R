...
encuesta_mrp <-
  readRDS("MrPDepartamental/CRI/2020/Data/encuesta_mrp.rds") %>%
  filter(edad != "1")
censo_mrp <-
  readRDS("MrPDepartamental/CRI/2020/Data/censo_mrp.rds") %>%
  filter(edad != "1")
tasa_desocupados <-
  readRDS("MrPDepartamental/CRI/2020/Data/tasa_desocupacion.rds")
statelevel_predictors_df <- tasa_desocupados
###################################################################
# Leyendo el modelo
fit <-
  cmdstan_model(stan_file = "MrPDepartamental/0Funciones/Multinivel8.stan",
                compile = TRUE)
...
encuesta_df_agg <-
  inner_join(encuesta_df_agg, statelevel_predictors_df, by = "depto")
#############################################
...
##################################################
X <-
  encuesta_df_agg %>% select(-matches("depto|empleo|tasa|^F|^X")) %>%
  model.matrix(~ -1 + ., data = .)  %>%
...
Xp <-
  censo_df %>% select(-matches("^n|depto|empleo|tasa|^F|^X")) %>%
  model.matrix(~ -1 + ., data = .)  %>%
...
fit_mcmc$save_object(file = "MrPDepartamental/CRI/2020/Data/fit_multinomial.rds")
