...
# Loading data ------------------------------------------------------------
encuesta_mrp <- readRDS("MrPDepartamental/CHL/2020/Data/encuesta_mrp.rds") %>% 
  filter(edad != "1")
censo_mrp <- readRDS("MrPDepartamental/CHL/2020/Data/censo_mrp.rds") %>% 
  filter(edad != "1")
tasa_desocupados <- readRDS("MrPDepartamental/CHL/2020/Data/tasa_desocupacion.rds")

fit <- readRDS(file = "MrPDepartamental/CHL/2020/Data/fit_multinomial.rds")
...
jpeg(file = "MrPDepartamental/CHL/2020/Output/Plot_Bench_gk.jpeg")
hist(gk)
dev.off()
poststrat_df$gk <- gk

saveRDS(poststrat_df %>% select(!matches("_\\d{,2}$"), matches("theta_")), 
        "MrPDepartamental/CHL/2020/Data/poststrat_df.RDS")
