...
data {
  ...
  int<lower=1> nGrupos; // número efectos aleatroios de 1|depto 
  int<lower=1> P; // categorías
  int<lower=1> K; // cantidad de regresores
  int idGrupos[D];   // Ids de para efectos aleatorios
...
transformed parameters {
  ...
  for(d in 1:D){
    num[d, 1] = 1;
    num[d, 2] = exp(X[d, ] * beta[1, ]' + u[1, idGrupos[d]]) ;
    num[d, 3] = exp(X[d, ] * beta[2, ]' + u[2, idGrupos[d]]) ;
