...
poststrat_df <- readRDS("COL/2020/1.Ingreso/Data/poststrat_df.RDS") %>% 
  filter(anoest != "99") %>% 
  mutate(.groups = NULL)
tasa_desocupados <- readRDS("COL/2020/1.Ingreso/Data/tasa_desocupacion.rds")
fit <- readRDS("COL/2020/1.Ingreso/Data/fit_bayes.rds")
...
openxlsx::write.xlsx(tablas, file = "COL/2020/1.Ingreso/Output/tablas_ECM.xlsx", 
                     overwrite = TRUE)
