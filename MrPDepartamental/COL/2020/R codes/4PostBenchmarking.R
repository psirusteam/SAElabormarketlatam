...
encuesta_mrp <- readRDS("MrPDepartamental/COL/2020/Data/encuesta_mrp.rds") %>% 
  filter(edad != "1")
censo_mrp <- readRDS("MrPDepartamental/COL/2020/Data/censo_mrp.rds") %>% 
  filter(edad != "1") %>% select(-.groups)
tasa_desocupados <- readRDS("MrPDepartamental/COL/2020/Data/tasa_desocupacion.rds")

fit <- readRDS("MrPDepartamental/COL/2020/Data/fit_multinomial.rds")
...
###########################################
###########################################
names_cov <- c("area",  "sexo",  "edad" , "etnia")
...
saveRDS(poststrat_df %>% select(!matches("_\\d{,2}$"), matches("theta_")), 
        "MrPDepartamental/COL/2020/Data/poststrat_df.RDS")
