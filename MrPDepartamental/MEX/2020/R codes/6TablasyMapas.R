...
## Leer Shape del pais
ShapeSAE <- read_sf("MrPDepartamental/MEX/2020/ShapeDeptoMEX/México_Estados.shp")

ShapeSAE %<>% mutate(depto = gsub(pattern = "MX", replacement =  "", x = CODIGO))

## Ajustando codigo de departamento

### cod de departamento en la shape
cod_shape <- ShapeSAE %>% as.data.frame() %>% select(depto, ESTADO) %>%
  arrange(depto)
...
ShapeSAE %<>% mutate(depto = case_when(depto == "07" ~ "05",
                                       depto == "08" ~ "06",
                                       depto == "05" ~ "07",
                                       depto == "06" ~ "08",
                                       TRUE ~ depto))
...
openxlsx::write.xlsx(dat_df, 
                     file = "MrPDepartamental/MEX/2020/Output/tablas.xlsx", 
                     overwrite = TRUE)
openxlsx::openXL("MrPDepartamental/MEX/2020/Output/tablas.xlsx")
